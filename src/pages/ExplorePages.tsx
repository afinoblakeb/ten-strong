...
export function CalendarPage() {
  ...
  return <div className="page"><div className="eyebrow">The whole arc</div><h1>90-day calendar</h1><p className="lede small">Your whole challenge at a glance.</p><div className="calendar-legend">{CALENDAR_LEGEND.map(([glyph,label])=><span key={label}><b aria-hidden>{glyph}</b> {label}</span>)}</div>{phases.map(phase=><section className="calendar-phase" key={phase.id}><div><span>Phase {phase.id} · Days {phase.start}–{phase.end}</span><h2>{phase.name}</h2></div><div className="day-grid" role="list" aria-label={`Phase ${phase.id}: ${phase.name}`}>{fullProgram.filter(d=>d.phaseId===phase.id).map(day=>{const status=calendarDayStatus(byDay.get(day.day),day.day,current);return <div className={calendarCellClass(status,day.kind)} role="listitem" aria-current={day.day===current?'date':undefined} aria-label={`Day ${day.day}, ${day.title}, ${calendarStatusLabel(status)}`} key={day.day}><strong aria-hidden>{day.day}</strong><span aria-hidden>{calendarGlyph(status,day.kind)}</span></div>})}</div></section>)}</div>
}
